...
export const createColumn = async ({
  ...
  const toastId = _toastId ?? toast.loading(`正在创建列 "${payload.name}"...`)
  try {
    ...
    if (!skipSuccessMessage) {
      toast.success(`成功创建了列“${formattedPayload.name}”`, { id: toastId })
    }
...
export const updateColumn = async ({
  ...
  try {
    ...
    if (!skipSuccessMessage) toast.success(`成功更新了列“${originalColumn.name}”`)
  } catch (error: any) {
    return { error }
  }
}
