...
export const SortPopoverPrimitive = ({
  ...
  const displayButtonText =
    buttonText ??
    (localSorts.length > 0
      ? `已按 ${localSorts.length} 条规则${localSorts.length > 1 ? '' : ''}排序`
      : '排序')
    ...
    <div className="space-y-2 py-2">
      ...
      {localSorts.length === 0 && (
        <div className="space-y-1 px-3">
          <h5 className="text-sm text-foreground-light">当前视图未应用任何排序规则</h5>
          <p className="text-xs text-foreground-lighter">在下方添加一栏排序视图中的数据</p>
        </div>
      )}
      ...
      <div className="px-3 flex flex-row justify-between">
        {dropdownOptions && dropdownOptions.length > 0 ? (
          ...
          >
            ...
            >
              <span>选择{localSorts.length > 1 ? '另一' : '一'}列排序</span>
            </Button>
          </DropdownControl>
        ) : (
          <p className="text-sm text-foreground-light">已添加所有列</p>
        )}
