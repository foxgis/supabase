                      ...
                      toast.error(
                        capitalize(
                          error?.originalResponse?.getBody() ||
                            `上传文件 ${file.name} 失败：${metadata.mimetype} 不被允许`
                        ),
                        {
                          description: `允许的 MIME 类型: ${state.selectedBucket.allowed_mime_types?.join(', ')}`,
                        }
                      )
                      break
                    case 413:
                      // Payload too large
                      toast.error(
                        `上传文件 ${file.name} 失败：文件大小超过了存储桶的上传限制。`
                      )
                      break
                    case 409:
                      // Resource already exists
                      toast.error(`上传文件 ${file.name} 失败：文件名称已存在。`)
                      break
                  }
                } else {
